...
inline int ComputeKktNullspaceHalfspaceTpl(
...
inline Real GdDerivativeTpl(const KinematicState<dim>& state1,
  ...
  switch (settings.twist_frame) {
    case TwistFrame::Spatial:
      return GdDerivativeImpl<TwistFrame::Spatial>(state1, state2, out, dd);
    case TwistFrame::Hybrid:
      return GdDerivativeImpl<TwistFrame::Hybrid>(state1, state2, out, dd);
    default:  // Body
      return GdDerivativeImpl<TwistFrame::Body>(state1, state2, out, dd);
  }
}
...
inline void GdGradientTpl(const Transformr<dim>& tf1,
                          const Transformr<dim>& tf2, const Settings& settings,
                          const Output<dim>& out, TotalDerivative<dim>& td) {
  switch (settings.twist_frame) {
    case TwistFrame::Spatial:
      return GdGradientImpl<TwistFrame::Spatial>(tf1, tf2, out, td);
    case TwistFrame::Hybrid:
      return GdGradientImpl<TwistFrame::Hybrid>(tf1, tf2, out, td);
    default:  // Body
      return GdGradientImpl<TwistFrame::Body>(tf1, tf2, out, td);
  }
}
 ...
 */
template <int dim, class C1>
inline bool FactorizeKktSystemHalfspaceTpl(
    const C1* set1, const Transformr<dim>& tf1, const Halfspace<dim>* /*set2*/,
    const Transformr<dim>& tf2, const Settings& settings,
    const Output<dim>& out, DirectionalDerivative<dim>& dd) {
  static_assert(detail::ConvexSetValidator<dim, C1>::valid,
...
inline void GdJacobianTpl(const Transformr<dim>& tf1,
  ...
  for (int i = 0; i < tw_dim; ++i) {
    ...
    set_derivatives(i, td.d_normal_tf1, td.d_z1_tf1, td.d_z2_tf1, td.d_gd_tf1);
    state1.tw(i) = Real(0.0);
  }

  for (int i = 0; i < tw_dim; ++i) {
    state2.tw(i) = Real(1.0);
    set_derivatives(i, td.d_normal_tf2, td.d_z1_tf2, td.d_z2_tf2, td.d_gd_tf2);
